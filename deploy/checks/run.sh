...
# 4000, 4001 and 55432. it takes about six minutes, most of it waiting for
...
# partition, and a test binary that takes six minutes is a test binary nobody
# runs.

set -uo pipefail
here="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"

checks=(image.sh stack.sh partition.sh handover.sh one-fire-per-occurrence.sh)
failed=()
