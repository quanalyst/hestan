...
through a constructor that reads better at a call site:
...
as `1.2 GB` and `3.4` as `3.4s`. anything computing over metadata reads all
five as one number, so a count that grew by 37 and a size that shrank by 4%
are the same arithmetic over different units. byte sizes are printed in
decimal units (kB, MB, GB, TB), the way storage and warehouses quote them.
