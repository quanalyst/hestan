...
```toml
hestan = { version = "0.1.0-alpha.2", features = ["dbt"] }
```
...
- **stdout and stderr are captured**, line by line, under the op and the
  attempt — the same [subprocess capture](logs.md#subprocess-capture) an
  isolated op's child gets, and subject to the same caps. dbt is chatty and
