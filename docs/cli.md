...
`run --wait` returns an exit code suitable for scripts and scheduled commands.
...
ok    hestan     0.2.5 in this deployment's binary, linux/aarch64,
...
| colours | group or origin labels the [web ui](web-ui.md#shade) draws on the same one of its six shades. the [hue](assets.md#hue) is a hash of one name, so nothing reading one name at a time can stop two of them landing together; this names them and `Asset::hue(n)` moves one. past six labels the pigeonhole decides it, and it says so as a fact rather than as a note nobody can act on | the asset graph, so the deployment's own binary |
